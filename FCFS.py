...
def fcfs(processes):
    ...
    while completed_processes < n:
        # Find the next process that has arrived
        ready_process = None
        for p in processes:
        ...
        if ready_process:
            ...
            current_time += ready_process['burst_time']
            ready_process['remaining_time'] = 0
            ready_process['completion_time'] = current_time
            ready_process['turnaround_time'] = ready_process['completion_time'] - ready_process['arrival_time']
            ready_process['waiting_time'] = ready_process['turnaround_time'] - ready_process['burst_time']

            total_waiting_time += ready_process['waiting_time']
            total_turnaround_time += ready_process['turnaround_time']
            total_completion_time += ready_process['completion_time']
            completed_processes += 1

            print(f"Process {ready_process['process_id']} completed at time {current_time}.")
            print(f"step: {current_time}, process: {ready_process['process_id']}, remaining time: {ready_process['remaining_time']}")
        else:
...
# Call the FCFS function to calculate the scheduling details
if processes:  # Only run if processes were successfully loaded
    avg_waiting_time, avg_turnaround_time, avg_completion_time = fcfs(processes)

    # Print the process details after the FCFS function has been executed
